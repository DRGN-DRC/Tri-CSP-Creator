...
gimpExeName = "gimp-console-2.8.exe"
...
class triCspCreator( object ):
	...
	def __init__( self, parent, dnd ):
		...
		#self.configFileEntry.bind( '<Return>', self.openConfig )
		ttk.Button( self.maskRow, text='Open', command=self.loadMask ).pack( side='left', padx=3 )
		maskImage = PhotoImage(file='imgs/mask.gif')
		self.maskWidget = ttk.Label( self.maskRow, image=maskImage, style='newLabel.TLabel' )
		self.maskWidget.image = maskImage # Keeps a reference to the image to prevent garbage collection
		self.maskWidget.pack( side='left' )
		...
		#self.configFileEntry.bind( '<Return>', self.openConfig )
		ttk.Button( self.configRow, text='Open', command=self.loadConfig ).pack( side='left', padx=3 )
		...
		self.fileIconWidget.pack( side='left' )
		...
		#self.configFileEntry.bind( '<Return>', self.openConfig )
		ttk.Button( self.outputPathRow, text='Choose Directory', command=self.loadOutputPath ).pack( side='left', padx=10, ipadx=5 )
		# fileIconImage = PhotoImage(file='imgs/fileIcon.gif')
		# self.fileIconWidget = ttk.Label( self.outputPathRow, image=fileIconImage, style='newLabel.TLabel' )
		# self.fileIconWidget.image = fileIconImage # Keeps a reference to the image to prevent garbage collection
		# self.fileIconWidget.pack( side='left' )
		...
		#buttonsGroup = ttk.Frame( self.buttonsRow )
		...
		ttk.Checkbutton( self.buttonsRow, text='  Preserve Original\n  Resolution', variable=self.saveHighRes, style='newCheckbox.TCheckbutton' ).grid( row=1, column=0, ipadx=15, sticky='w' )
		ttk.Label( self.buttonsRow, text='Halp!', style='newLabel.TLabel' ).grid( row=1, column=1 )
	...
	def resizeTextFields( self, event ):
		...
		while focusedTextWidget['width'] < self.pathsInputWidth * 2 and self.mainFrame.focus_get() == focusedTextWidget: # Second argument causes loop to end if another Text gains focus
			for inputField in widgetsToResize:
				...
				#stepDistance = int( math.ceil( ( targetWidth - currentWidth ) / 30.0 ) )
				#print 'total distance diff:', targetWidth - currentWidth, 'stepDistance:', stepDistance, 'targetWidth:', targetWidth
	...
	def dndHandler( self, event ):
		# The paths that this event returns are in one string, each enclosed in {} brackets (if they contain a space) and separated by a space. Turn this into a list.
		#filepaths = [path.strip('{}') for path in event.data.split('} {')]
	...
	def fileHandler( self, event, filepaths ):
		...
		if focusedWidget == self.maskRow:
			...
			elif len( filepaths ) == 1: self.outputPath.set( filepaths[0] )

		else:
			# Remove existing line labels along the left & right headers
			# for widget in self.mainFrame.winfo_children():
			# 	gridColumn = widget.grid_info()['column']
			# 	if gridColumn == '0' or gridColumn == '4': widget.destroy()
			...
			#else: # Remove the line number labels
				# for widget in self.mainFrame.winfo_children():
				# 	if widget.grid_info()['column'] == '0' or widget.grid_info()['column'] == '4': widget.destroy()
			# else:
			# 	# Placeholder labels (preserves space along the sides of the Text widgets for later addition of line labels)
			# 	ttk.Label(mainFrame, text='', style='newLabel.TLabel', width=3 ).grid( row=0, column=0, padx=6 )
			# 	ttk.Label(mainFrame, text='', style='newLabel.TLabel', width=3 ).grid( row=0, column=4, padx=6 )
			...
			# Modify the Create CSP button text
			if mostPaths > 1:
	...
	def validatePath( self, event, widget=None ):
		...
		if widget == self.maskFileEntry:
			...
			self.maskWidget.configure( image=maskImage )
			self.maskWidget.image = maskImage
	...
	def loadOutputPath( self, outputPath='' ):
		...
		if outputPath:
			...
			if os.path.isdir( normalizedPath ): self.outputPath.set( normalizedPath )
	...
	def createCSP( self ):
		...
		if not self.creationInProgress:
			# Check for a config file, and process it if present
			...
			if not configPath:
				...
				time.sleep( 1.5 )
				#ttk.Label( self.buttonsRow, text='Processing:', style='newLabel.TLabel', width=17, anchor='n' ).pack( side='left' )
				...
				extraImages, skippedSets = self.processGimpAutomation( leftImagePaths, centerImagePaths, rightImagePaths, maskPath, '', outPath, configuration, self.saveHighRes.get() )
	...
	def clearLineNumberLabels( self, replace=True ):
		# Remove existing line labels along the left & right headers
		for widget in self.mainFrame.winfo_children():
		...
		# if replace:
		# 	# Placeholder labels (preserves space along the sides of the Text widgets for later addition of line labels)
		# 	ttk.Label( self.mainFrame, text='', style='newLabel.TLabel', width=3 ).grid( row=0, column=0, padx=6 )
		# 	ttk.Label( self.mainFrame, text='', style='newLabel.TLabel', width=3 ).grid( row=0, column=4, padx=6 )
	...
	def processGimpAutomation( self, leftImagePaths, centerImagePaths, rightImagePaths, maskImagePath, configFilePath, outputPath, configuration, saveHighRes ):
		...
		#for paths in ( leftImagePaths, centerImagePaths, rightImagePaths ):
		...
		for i in range( cspsToCreate ):
			...
			returnCode, outputStream = callGimpAutomation( leftImagePath, centerImagePath, rightImagePath, maskImagePath, configFilePath, outputPath, configuration, saveHighRes )
...
def initializeGui():
	root = Tk()
	#root.tk_setPalette( background='#e7e7ff' )
	dnd = TkDND( root )
	...
	#root.minsize( width=640, height=425 )
...
def parseArguments():
	...
	subparsers = parser.add_subparsers( title='Run modes', dest='mode',
										description='You can use this program in one of three ways, via "20XX-CSP-Creator.py gui ...", "20XX-CSP-Creator.py cmd ...", or  "20XX-CSP-Creator.py cmd-config ..."' )
	...
	cmdModeParser = subparsers.add_parser( 'cmd',
		help='Run this program via command line only, in which case image paths and most other arguments are required. Run "20XX-CSP-Creator.py cmd -h" to see a list of all arguments.' )
	...
	#cmdModeParser.add_argument( '-s', '--showFinished', action='store_true', help='' )
	...
	#cmdConfigModeParser.add_argument( '-s', '--showFinished', action='store_true', help='' )
...
def parseConfigurationFile( configFilepath ):
	...
	# 					('sideImagesXOffset', None), ('sideImagesYOffset', None), ('sideImagesScaling', None) ])

	configuration = OrderedDict([ ('threshold', 50), ('centerImageXOffset', 0), ('centerImageYOffset', 0), ('centerImageScaling', 1.0),
...
def processGimpAutomation( leftImagePaths, centerImagePaths, rightImagePaths, maskImagePath, configFilePath, outputPath, configuration, saveHighRes ):
	# Get the length of the shortest list, this will be how many CSPs will be created
	cspsToCreate = 0
	for pathsList in ( leftImagePaths, centerImagePaths, rightImagePaths ):
		totalImages = len( pathsList )
	...
	for i in range( cspsToCreate ):
		...
		if not os.path.exists( leftImagePaths[i] ) or not os.path.exists( centerImagePaths[i] ) or not os.path.exists( rightImagePaths[i] ):
			print 'Image set ' + str(i+1) + ' could not be processed because one of the images in the set could not be found.'
			continue
...
def preparePathForGimpCmd( originalFilepath ): # GIMP via command line is VERY picky.
	#if ':' not in originalFilepath:  # Probably a relative path.
	return originalFilepath.replace('\\', '/')


def callGimpAutomation( leftImagePath, centerImagePath, rightImagePath, maskImagePath, configFilePath, outputPath, configuration, saveHighRes ):
	...
	command = ( 'start /b /d "' + gimpBinaryDirectory + '" ' + gimpExeName + ' -d -f -i -b "(python-fu-create-tri-csp 1 ' # Extra param, 1, to run in NON-INTERACTIVE mode
	 	'\\"' + leftImagePath + '\\" '
	 	'\\"' + centerImagePath + '\\" '
	 	'\\"' + rightImagePath + '\\" '
	 	'\\"' + maskImagePath + '\\" '
	 	'\\"' + configFilePath + '\\" '
	 	'\\"' + outputPath + '\\" '
	 	#+ str( threshold ) + str( centerXOffset ) + str( centerYOffset ) + str( centerImageScaling ) + str( sideImagesXOffset ) + str( sideImagesYOffset ) + str( threshold ) + str( threshold ) 
	...
	# :: -b "" specifies script-fu command to be run in gimp.
...
def cmdChannel( command, standardInput=None, shell=True ): # IPC (Inter-Process Communication) to command line.
	#tkMessageBox.showinfo('', command)
	#if standardInput: stdin = subprocess.PIPE
	#else: stdin = None

	#try:
		#outputStream = subprocess.check_output( command, shell=True, stdin=stdin, stderr=subprocess.STDOUT, creationflags=0x08000000 ) # shell=True gives access to all shell features.
...
def main():
	tkMessageBox.showinfo( message='test' )	
	args = parseArguments() # Doesn't work after compilation?

	# tkMessageBox.showinfo( message=args.mode )	
	# tkMessageBox.showinfo( message=args )	

	if args.mode == 'cmd' or args.mode == 'cmd-config':
		configuration = getConfigurationFromCmd( args )

		# print '\nRecieved arguments:\n', args
		# print '\nConfiguration determined:\n', configuration

		processGimpAutomation( args.leftImagePaths, args.centerImagePaths, args.rightImagePaths, args.maskImagePath, '', args.outputPath, configuration, args.saveHighRes )
